...
module.exports = () => {
    return new Promise ((resolve, reject) => {
        console.log('==== Starting Noi Mascotas cat food web scraper ====')
        let startTimer = timer();
        // Init scraper
        x(
            'https://www.noi.la/mascotte/gato/alimentos-gatos/',
            'div.product',
            [{
        ...
        ((err, data) => {
            if (err) {
                console.log('Error fron Noi cat food web scraper...');
                reject(err)
            } else {
                let endTimer = timer(startTimer);
                console.log("== Noi Mascotas cat food web scraper completed in: " + endTimer + " ms ==")
                resolve(data)
            }
        })
    })
