...
module.exports = () => {
    return new Promise ((resolve, reject) => {
        console.log('==== Starting Pet Happy dog food web scraper ====')
        let startTimer = timer();
        // Init scraper
        x(
            'https://www.pethappy.cl/perros-2/alimentos',
        ...
        ((err, data) => {
            if (err) {
                console.log('Error fron Pet Happy dog food web scraper...')
                reject(err)
            } else {
                let endTimer = timer(startTimer)
                console.log("== Pet Happy dog food web scraper completed in: " + endTimer + " ms ==")
                resolve(data)
            }
        })
    })
