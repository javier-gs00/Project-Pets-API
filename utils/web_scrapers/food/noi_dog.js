...
module.exports = () => {
    return new Promise ((resolve, reject) => {
        console.log('==== Starting Noi Mascotas dog food web scraper ====')
        let startTimer = timer();
        // Init scraper
        x(
            'https://www.noi.la/mascotte/perros/alimentos-perros/',
            'div.product',
            [{
        ...
        ((err, data) => {
            if (err) {
                console.log('Error fron Noi dog food web scraper...');
                reject(err)
            } else {
                let endTimer = timer(startTimer);
                console.log("== Noi Mascotas dog food web scraper completed in: " + endTimer + " ms ==")
                resolve(data)
            }
        })
    })
