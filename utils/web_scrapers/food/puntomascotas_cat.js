...
module.exports = () => {
    return new Promise ((resolve, reject) => {
        console.log('==== Starting Punto Mascotas cat food web scraper ====')
        let startTimer = timer();
        // Init scraper
        x(
            'http://www.puntomascotas.cl/category.php?id_category=8',
        ...
        ((err, data) => {
            if (err) {
                console.log('Error fron Punto Mascotas cat food web scraper...')
                reject(err)
            } else {
                let endTimer = timer(startTimer)
                console.log("== Punto Mascotas cat food web scraper completed in: " + endTimer + " ms ==")
                resolve(data)
            }
        })
    })
