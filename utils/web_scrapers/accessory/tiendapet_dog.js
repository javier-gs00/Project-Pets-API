...
function accessoriesCrawler () {
    return new Promise ((resolve, reject) => {
        console.log('==== Starting Tienda Pet dog accessories web scraper ====')
        let startTimer = timer();
        // Init scraper
        x(
            'https://www.tiendapet.cl/catalogo/perro/accesorios/',
            'div.block-producto',
            [{
        ...
        .limit(17) // Pages to crawl limit
        ((err, data) => {
            if (err) {
                console.log('Error fron Tienda Pet dog accessories web scraper...')
                reject(err)
            } else {
                let endTimer = timer(startTimer)
                console.log("== Tienda Pet dog accessories web scraper completed in: " + endTimer + " ms ==")
...
function toysCrawler () {
    return new Promise ((resolve, reject) => {
        console.log('==== Starting Tienda Pet dog toys web scraper ====')
        let startTimer = timer();
        // Init scraper
        x(
            'https://www.tiendapet.cl/catalogo/perro/juguetes/',
            'div.block-producto',
            [{
                name: 'h5 | parseHexUnicode',
                href: 'a.catalogo_click_detail@href',
        ...
        ((err, data) => {
            if (err) {
                console.log('Error fron Tienda Pet dog toys web scraper...')
                reject(err)
            } else {
                let endTimer = timer(startTimer)
                console.log("== Tienda Pet dog toys web scraper completed in: " + endTimer + " ms ==")
                let products = handleTableData(data)
                resolve(products)
            }
        })
