...
module.exports = () => {
    return new Promise ((resolve, reject) => {
        ...
        Promise.all(resolvedPromisesArray).then(categoryArray => {
            ...
            // so we have to iterate on each array and push each product into the new array
...
// Web crawler for /accesorios. Returns an array of objects
function accessoriesCrawler () {
    return new Promise ((resolve, reject) => {
        console.log('==== Starting Tienda Pet cat accessories web scraper ====')
        let startTimer = timer();
        // Init scraper
        x(
            'https://www.tiendapet.cl/catalogo/gato/accesorios/',
            'div.block-producto',
            [{
        ...
        .limit(10) // Pages to crawl limit
        ((err, data) => {
            if (err) {
                console.log('Error fron Tienda Pet cat accessories web scraper...')
                reject(err)
            } else {
                let endTimer = timer(startTimer)
                console.log("== Tienda Pet cat accessories web scraper completed in: " + endTimer + " ms ==")
...
// Web crawler for /juguetes. Returns an array of objects
function toysCrawler () {
    return new Promise ((resolve, reject) => {
        console.log('==== Starting Tienda Pet cat toys web scraper ====')
        let startTimer = timer();
        // Init scraper
        x(
            'https://www.tiendapet.cl/catalogo/gato/juguetes/',
            'div.block-producto',
            [{
                name: 'h5 | parseHexUnicode',
                href: 'a.catalogo_click_detail@href',
        ...
        ((err, data) => {
            if (err) {
                console.log('Error fron Tienda Pet cat toys web scraper...')
                reject(err)
            } else {
                let endTimer = timer(startTimer)
                console.log("== Tienda Pet cat toys web scraper completed in: " + endTimer + " ms ==")
                let products = handleTableData(data)
                resolve(products)
            }
        })
