...
module.exports = () => {
    return new Promise ((resolve, reject) => {
        console.log('==== Starting Pet Happy cat accessory web scraper ====')
        let startTimer = timer();
        // Init scraper
        x(
            'https://www.pethappy.cl/gatos-2/accesorios',
        ...
        ((err, data) => {
            if (err) {
                console.log('Error fron Pet Happy cat accessory web scraper...')
                reject(err)
            } else {
                let endTimer = timer(startTimer)
                console.log("== Pet Happy cat accessory web scraper completed in: " + endTimer + " ms ==")
                resolve(data)
            }
        })
    })
