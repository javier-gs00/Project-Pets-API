...
module.exports = () => {
    return new Promise ((resolve, reject) => {
        console.log('==== Starting Punto Mascotas food web scraper ====')
        let startTimer = timer();
        // Init scraper
        x(
            'http://www.puntomascotas.cl/category.php?id_category=13',
        ...
        .limit(13) // Pages to crawl limit
        ((err, data) => {
            if (err) {
                console.log('Error fron Punto Mascotas food web scraper...')
                reject(err)
            } else {
                let endTimer = timer(startTimer)
                console.log("== Punto Mascotas food web scraper completed in: " + endTimer + " ms ==")
                resolve(data)
            }
        })
    })
