...
module.exports = () => {
    return new Promise ((resolve, reject) => {
        console.log('==== Starting Tienda Pet cat medicine web scraper ====')
        let startTimer = timer();
        // Init scraper
        x(
            'https://www.tiendapet.cl/catalogo/gato/farmacia/',
            'div.block-producto',
            [{
        ...
        ((err, data) => {
            if (err) {
                console.log('Error fron Tienda Pet cat medicine web scraper...')
                reject(err)
            } else {
                let endTimer = timer(startTimer)
                console.log("== Tienda Pet cat medicine web scraper completed in: " + endTimer + " ms ==")
